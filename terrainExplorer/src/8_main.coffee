...
game = (canvas_container) ->
	...
	commands = ( -> 
		cmd = {}
		cmd[UP] = -> explorer.move({x: 0, y: -1})
		cmd[DOWN] = -> explorer.move({x: 0, y: 1})
		cmd[LEFT] = -> explorer.move({x: -1, y: 0})
		cmd[RIGHT] = -> explorer.move({x: 1, y: 0})
...
#					terrain.lineHit(150+(x*12), 150+(y*12), 150+(x*12)+x, 150+(y*12)+y)
	...
	$(document).keydown((eventInfo) -> keys[eventInfo.which] = true)
	$(document).keyup((eventInfo) -> keys[eventInfo.which] = false)
	...
	explorer = new Explorer({x: 20, y: 100}, terrain)
